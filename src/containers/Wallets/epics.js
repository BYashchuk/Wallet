...
export const getTransactionsBySelectedCoinEpic = (action$, state$) => action$.pipe(
    ...
    mergeMap(([action, state]) => {
        ...
        return getApiProviderAsync(blokchain).pipe(
            ...
            map(transactions => transactions.map(rawTransaction => new Transaction(rawTransaction)))
        )
    }),
    map(transactionsBySelectedCoin => getTransactionsBySelectedCoin(transactionsBySelectedCoin))
)
